...
export class Dashboard {
  ...
  return forkJoin({
    ...
    ventasAyer: this.http.get<{ total: number }>(`${this.apiUrl}/venta/ventas-ayer`).pipe(map(r => r.total)),
    totalClientes: this.http.get<any>(`${this.apiUrl}/cliente?limit=1`).pipe(map(r => r.meta.total)),
    clientesNuevos: this.http.get<{ total: number }>(`${this.apiUrl}/cliente/clientes-nuevos`).pipe(map(r => r.total)),
    ...
    ventasSemana: this.http.get<any[]>(`${this.apiUrl}/venta/ventas-semana`),
    ingresosMensuales: this.http.get<any[]>(`${this.apiUrl}/venta/ingresos-mensuales`),
  });
}
}